    ...
    {   
        ...
        foreach (Item item in items)
        {
            if (item.IsExpired())
            {
                ...
                Console.WriteLine("Did not remove any items");
            }
        }
        foreach (Item expiredItem in expiredItems)
        {
            RemoveItem(expiredItem);
            Console.WriteLine("Removed expired item.");
        }
    }
}
...
public class Test
{
    public static void Main(string[] args)
    {
        ...
        Inventory inventory = new Inventory(); // init inventory
        FoodItem foodItem = new FoodItem("Milk", 69, DateTime.Now.AddDays(-1)); // init item
        inventory.AddItem(foodItem); // add item
        inventory.PrintInventory(); // pritning inventory
        Console.WriteLine(foodItem.IsExpired() ? "Expired" : "Not expired"); // expired? Yes
        inventory.RemoveExpiredFoods(); // removing
        inventory.PrintInventory(); // Printing, should be removed
